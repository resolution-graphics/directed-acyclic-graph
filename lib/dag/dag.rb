module Dag
  ...
  def dag_links(options = {})
    ...
    write_inheritable_attribute :dag_options, conf
    ...
    if dag_polymorphic?
      ...
      belongs_to :ancestor, :foreign_key => ancestor_id_column_name, :class_name => dag_options[:node_class_name]
      belongs_to :descendant, :foreign_key => descendant_id_column_name, :class_name => dag_options[:node_class_name]
    ...
    [count_column_name].each do |column|
      module_eval <<-"end_eval", __FILE__, __LINE__
						def #{column}=(x)
							raise ActiveRecord::ActiveRecordError, "ERROR: Unauthorized assignment to #{column}: it's an internal field handled by directed acyclic graph code."
						end
      end_eval
    end
  end
  ...
  def has_dag_links(options = {})
    ...
    if dag_link_class.dag_polymorphic?
      self.class_eval <<-EOL
